...
def search_action():
    ...
    elif choice == '4':  # Vẽ biểu đồ điểm
        result = plot_student_grades(data, student_id)
        if result:
...
def plot_student_grades(data, student_id):
    """Vẽ biểu đồ đường thể hiện điểm của một sinh viên."""
    if data.size == 0:
        return "Dữ liệu không được tải."
    ...
    # Tách tên môn học và điểm số
    subjects = student_data[:, 2]
    ...
    plt.xlabel('Môn học')
    plt.ylabel('Điểm')
    plt.title(f'Biểu đồ điểm của sinh viên có ID {student_id}')
    plt.grid(True)
...
def main():
    ...
    tk.Radiobutton(root, text="Vẽ biểu đồ điểm của sinh viên", variable=choice_var, value='4').pack(anchor='w')
