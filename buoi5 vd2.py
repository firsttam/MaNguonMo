...
# Hàm train mô hình
def train_model():
    global model, df, mse_values, mae_values, rmse_values, y_test, y_predict
    if df is None:
    ...
    mse_values[algorithm] = mean_squared_error(y_test, y_predict)
    mae_values[algorithm] = mean_absolute_error(y_test, y_predict)
    rmse_values[algorithm] = np.sqrt(mse_values[algorithm])

    # Hiển thị kết quả
    mse_label.config(text=f"MSE: {mse_values[algorithm]:.2f}")
    mae_label.config(text=f"MAE: {mae_values[algorithm]:.2f}")
    rmse_label.config(text=f"RMSE: {rmse_values[algorithm]:.2f}")

    # Lưu lại y_test và y_predict cho việc dự đoán
    global y_test_global, y_predict_global
    y_test_global = y_test
    y_predict_global = y_predict
...
def plot_error_distribution():
    if y_test_global is None or y_predict_global is None:
        messagebox.showerror("Lỗi", "Hãy train mô hình trước!")
        return

    # Tính sai số
    errors = abs(y_predict_global - y_test_global)
...
def plot_error_bars():
    if not mse_values or not mae_values or not rmse_values:
        messagebox.showerror("Lỗi", "Hãy train mô hình trước!")
        return

    # Vẽ biểu đồ cột
    labels = mse_values.keys()
    mse_vals = list(mse_values.values())
    rmse_vals = list(rmse_values.values())
    mae_vals = list(mae_values.values())

    x = np.arange(len(labels))  # the label locations
    width = 0.2  # the width of the bars

    fig, ax = plt.subplots()
    ax.bar(x - width, mse_vals, width, label='MSE', color='blue')
    ax.bar(x, rmse_vals, width, label='RMSE', color='orange')
    ax.bar(x + width, mae_vals, width, label='MAE', color='green')

    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax.set_ylabel('Giá trị lỗi')
    ax.set_title('Biểu đồ cột cho MSE, RMSE, MAE theo từng thuật toán')
    ax.set_xticks(x)
